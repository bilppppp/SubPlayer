...
export function ExportDialog({ segments, readableBlocks, filename, hasTranslation }: ExportDialogProps) {
  ...
  const handleExport = () => {
    ...
    exportSubtitles(exportData, format, baseName, includeTranslation && hasTranslation);
    setOpen(false);
  };
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button variant="outline" size="sm" className="border-black bg-white hover:bg-black hover:text-white" disabled={segments.length === 0}>
          <Download className="mr-1.5 h-3.5 w-3.5" />
          导出
        </Button>
      </DialogTrigger>
      <DialogContent className="sm:max-w-sm border-black bg-canvas">
        <DialogHeader>
          <DialogTitle className="font-mono uppercase tracking-wide">导出字幕</DialogTitle>
          <DialogDescription className="text-foreground/70">选择格式并下载字幕文件</DialogDescription>
        </DialogHeader>
        ...
        <Button onClick={handleExport} className="w-full border border-black bg-black text-neon hover:bg-black/90">
